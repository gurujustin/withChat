...
const Inner = styled.div<{ isPushed: boolean; showMenu: boolean; totalMenuHeight: number; showChat: boolean }>`
  flex-grow: 1;
  margin-top: ${({ totalMenuHeight }) => `${totalMenuHeight}px`};
  ...
  ${({ theme }) => theme.mediaQueries.nav} {
    ...
    margin-right: ${({ showChat }) => `${showChat ? "340" : "0"}px`};
      ...
      <Wrapper>
        ...
        <BodyWrapper mt={!subLinks ? `${totalTopMenuHeight + 1}px` : "0"}>
          ...
          <ChatPanel
            ...
            chatLayout={chat}
          />
          <Inner isPushed={isPushed} showMenu={showMenu} totalMenuHeight={totalTopMenuHeight} showChat={showChat}>
            {children}
            <Footer
              items={footerLinks}
              isDark={isDark}
